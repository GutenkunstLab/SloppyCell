...
net = IO.from_SBML_file('KHOL.xml', 'net1')                             # (@\label{code:SBML_import}@)
#net.set_var_ic('v1', 'v1_0') # Won't need given initial assignments.           # (@\label{code:SBML_fix}@)
import data_to_fit                                                                # (@\label{code:import_expt}@)
m = Model([data_to_fit.expt], [net])

params = KeyedList([('V1', 2.5), ('K1', 10), ('V2', 0.25),                      # (@\label{code:params} @)
                    ('KK3', 15), ('KK4', 15)])
