...
void integrate_stochastic_tidbit_(unsigned long* seed_ptr, int* reseed, double* time_ptr, int* dv, double* cv, double* rmsd_ptr, double* stop_time_ptr, double* trajectory) {
  ...
  short stch[2][2] = {{-1, 1},
                    {1, -1}};
  short depd[2+1][2] = {{1, 1},
                    {1, 1},
                    {1, 1}};

  double time = *time_ptr;
  double sd = (*rmsd_ptr)*(*rmsd_ptr)*2.;
  double stop_time = *stop_time_ptr;
  double dt=0.0;

  double dv0[2];
  int rxnInd = 2;
  double propensity, selection, props[2], av[0];
    double _sd = 0.0;
  if (*reseed) {init_genrand(seed);}

  for (i=0;i<2;i++) {dv0[i]=dv[i];}

  while (time < stop_time) {

    if (depd[rxnInd][0]) {props[0]=((A)*(x));}
    if (depd[rxnInd][1]) {props[1]=((B)*(y));}
   ...
   if (propensity<=0.0) {
    ...
    for (rxnInd=0; rxnInd<2; rxnInd++) {
      if (selection < props[rxnInd]) {break;}
      else {selection -= props[rxnInd];}}

    for (i=0;i<2;i++) {dv[i]+=stch[rxnInd][i];}
