...
double S3 = dynamicVars[4];
double S4 = dynamicVars[5];
...
alg_derivs_res[0] = (((0.0) + ((-(1.0))*(yp[(, <, _, a, s, t, ., I, n, d, e, x,  , o, b, j, e, c, t,  , a, t,  , 0, x, 7, f, 8, 8, 5, 0, 3, 0, 6, a, 0, 0, >, )]))) + (((Keq) + (1.0))*(yp[(, <, _, a, s, t, ., I, n, d, e, x,  , o, b, j, e, c, t,  , a, t,  , 0, x, 7, f, 8, 8, 5, 0, 2, a, a, c, d, 0, >, )])));
alg_derivs_res[1] = ((0.0) + ((1.0)*(yp[(, <, _, a, s, t, ., I, n, d, e, x,  , o, b, j, e, c, t,  , a, t,  , 0, x, 7, f, 8, 8, 5, 0, 2, a, a, b, 2, 0, >, )])));
alg_derivs_res[2] = ((0.0) + ((1.0)*(yp[(, <, _, a, s, t, ., I, n, d, e, x,  , o, b, j, e, c, t,  , a, t,  , 0, x, 7, f, 8, 8, 5, 0, 2, b, 4, a, 6, 0, >, )])));
}

void alg_res_func_(double *alg_vals, double *dynamicVars, double *time_ptr, double *constants, double *residual){
double time = *time_ptr;

double X0, X1, T, S1, S3, S4;
double cell, Keq, k1, k2;
double S2;
dynamicVars[3] = alg_vals[0];
dynamicVars[4] = alg_vals[1];
dynamicVars[5] = alg_vals[2];
...
int ii;
double local_dres_dc[36] = {0};
double local_dres_dcdot[36] = {0};
int row, col;
...
for(row = 0; row < 6; row++){
for(col = 0; col < 6; col++){
sens_res[row+6] += local_dres_dc[row + col*6]*dc_dp[col];}}
dres_dcdot_function_(time_ptr, sens_y, sens_yp, constants, local_dres_dcdot);
for(row = 0; row < 6; row++){
for(col = 0; col < 6; col++){
sens_res[row+6] += local_dres_dcdot[row + col*6]*dcdot_dp[col];}}
}

void res_function_logdv_(double *time_ptr, double *log_dv, double *log_yp, double *cj_ptr, double *residual, int *ires_ptr, double *constants, int *ipar){
double dynamicVars[6];
double yprime[6];
int ii;
for(ii = 0; ii < 6; ii++){
...
double S3 = dynamicVars[4];
double S3_deriv_wrt_time = yprime[4];
double S4 = dynamicVars[5];
double S4_deriv_wrt_time = yprime[5];
