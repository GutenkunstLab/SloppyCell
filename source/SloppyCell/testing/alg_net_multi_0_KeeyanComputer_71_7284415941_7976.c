...
alg_derivs_res[0] = 0.0 + -1.0*yp[2] + (Keq + 1.0)*alg_yp[0];
alg_derivs_res[1] = 0.0 + 1.0*alg_yp[1];
alg_derivs_res[2] = 0.0 + 1.0*alg_yp[2];
}
...
double *dc_dp = &sens_y[6];
double *dcdot_dp = &sens_yp[6];
double *local_dres_dp = &sens_res[6];
int ii;
for(ii = 0; ii < 6; ii++){
local_dres_dp[ii] = 0;}
switch(p_index)
