...
void dres_dk_(double *time_ptr, double *dynamicVars, double *yprime, double *constants, double *pd);
...
residual[0] = -(B*A*k*10.0) - yprime[0];
residual[1] = A*k - yprime[1];
}
...
int p_index = (int)constants[2];
double constants_only[2];
int jj;
for (jj = 0; jj < 2; jj++){
...
switch(p_index)
{
case 0 : dres_dk_(time_ptr, sens_y, sens_yp, constants_only, local_dres_dp);
break;
}
