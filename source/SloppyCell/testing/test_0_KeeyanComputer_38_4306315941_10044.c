...
double f(double x){return x + 4.0;}

double f_0(double x){return 1.0;}
...
int p_index = (int)constants[0];
double constants_only[0];
int jj;
for (jj = 0; jj < 0; jj++){
constants_only[jj] = constants[jj];}
double *dc_dp = &sens_y[1];
double *dcdot_dp = &sens_yp[1];
double *local_dres_dp = &sens_res[1];
int ii;
for(ii = 0; ii < 1; ii++){
local_dres_dp[ii] = 0;}
switch(p_index)
...
double local_dres_dc[1] = {0};
dres_dc_function_(time_ptr, sens_y, sens_yp, constants, local_dres_dc);
int row, col;
for(row = 0; row < 1; row++){
for(col = 0; col < 1; col++){
sens_res[row+1] += local_dres_dc[row + col*1]*dc_dp[col];}}
double local_dres_dcdot[1] = {0};
dres_dcdot_function_(time_ptr, sens_y, sens_yp, constants, local_dres_dcdot);
for(row = 0; row < 1; row++){
for(col = 0; col < 1; col++){
sens_res[row+1] += local_dres_dcdot[row + col*1]*dcdot_dp[col];}}
}

void res_function_logdv_(double *time_ptr, double *log_dv, double *log_yp, double *cj_ptr, double *residual, int *ires_ptr, double *constants, int *ipar){
double dynamicVars[1];
double yprime[1];
int ii;
for(ii = 0; ii < 1; ii++){
